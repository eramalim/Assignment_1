...
show tables;
desc actor; -- actor_id is primary key and last_name is foreign key
desc actor_award; -- actor_award_id is primary key and last_name is foreign key
desc address; -- address_id is primary key and city_id is foreign key
desc advisor; -- advisor_id is primary key
desc category; -- category_id is primary key 
desc city; -- city_id is primary key and country_id is foreign key
desc country; -- country_id is primary key 
desc customer; -- customer_id is primary key and store_id, last_name, address_id are foreign key
desc film; -- film_id is primary key and description, language_id, original_language_id are foreign key
desc film_actor; -- actor_id, film_id are primary key
desc film_category; -- film_id, category_id are primary key
desc film_text; -- film_id is primary key and title is foreign key
desc inventory; -- inventory_id is primary key and film_id, store_id are foreign key
desc investor; -- investor_id is primary key 
desc language; -- language_id is primary key 
desc payment; -- payment_id is primary key and customer_id, staff_id, rental_id are foreign key
desc rental; -- rental_id is primary key and rental_date, inventory_id, customer_id, staff_id are foreign key
desc staff; -- staff_id is primary key and address_id, store_id are foreign key
desc store; -- store_id is primary key address_id is foreign key and manager_staff_id is unique 
...
select active from customer where active <> 0;
